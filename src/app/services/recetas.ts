...
export class Recetas {
  ...
  constructor() {
    const recetasJSON: RecetaJSON[] = [
      ...
      },
      {
        ...
        dificultad: 'Fácil',
        tiempo_preparacion: '10 min',
        pasos: ['Tuesta almendras con aceite y romero.', 'Deja enfriar y sirve como aperitivo.'],
        ingredientes: [
          {
  ...
  //Método para eliminar receta de recetas guardadas
  eliminarReceta(id: string): void {
    this.recetas_guardadas = this.recetas_guardadas.filter((r) => r._id !== id);
  }

  // Comprobar si una receta ya está guardada
  esFavorita(id: string): boolean {
    return this.recetas_guardadas.some((r) => r._id === id);
  }
}
